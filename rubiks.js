(function(){
    ...
    $(document).ready(function() {
        $('.glube').each(function(){
            ...
            // controls         
            $(this).find('.reset-cube').click(function () {
                glube.reset();
                glube.currentAlgoState = 0;
            });
            $(this).find('.scramble-cube').click(function () {
                glube.scramble();
            });
            $(this).find('.run-alg').click(function () {
                ...
                if(glube.currentAlgoState == 0)
                    glube.doAlgorithm(moves);
                else
                    glube.doAlgorithm(moves.slice(glube.currentAlgoState, moves.length));
                glube.currentAlgoState = moves.length;
            });
            $(this).find('.backward-alg').click(function () {
                glube.isInitializing = false;
                var alg = $(this).parent().find('.algorithm').val();
                var moves = glube.parseAlgorithm(alg);

                var currentState = glube.currentAlgoState;
                if(currentState == 0)
                    return;

                var moveToDo = moves[currentState-1];
                moveToDo.inverse = !moveToDo.inverse;

                if(glube.doAlgorithm([moveToDo]))
                    glube.currentAlgoState = currentState-1;
            });
            $(this).find('.forward-alg').click(function () {
                glube.isInitializing = false;
                var alg = $(this).parent().find('.algorithm').val();
                var moves = glube.parseAlgorithm(alg);

                var currentState = glube.currentAlgoState;
                if(currentState >= moves.length)
                    return;
                if(glube.doAlgorithm([moves[currentState]]))
                    glube.currentAlgoState = currentState + 1;
            });
        });  
    });
